...
import com.wwr.apipost.config.domain.Value;
import com.wwr.apipost.parse.constant.DocumentTags;
import com.wwr.apipost.parse.constant.JavaConstants;
import com.wwr.apipost.parse.constant.SpringConstants;
import com.wwr.apipost.parse.model.Jsr303Info;
import com.wwr.apipost.parse.model.TypeParseContext;
import com.wwr.apipost.parse.util.InternalUtils;
import com.wwr.apipost.parse.util.PsiSwaggerUtils;
import com.wwr.apipost.util.psi.*;
...
public class ParseHelper {
    ...
    public String getApiSummary(PsiMethod psiMethod) {
        ...
        // 从doc中解析描述
        if (comment != null) {
            summary = getSummaryFromDescriptionField(psiMethod);
            if (StringUtils.isNotBlank(summary)) {
                return trim(summary);
            }
            summary = getSummaryByDocFirstLine(comment);
            if (StringUtils.isNotBlank(summary)) {
                return trim(summary);
            }
        }
    ...
    private String getSummaryByDocFirstLine(PsiDocComment comment) {
    ...
    private String getSummaryFromDescriptionField(PsiMethod psiMethod) {
        String summary = null;
